...
const STRATEGY_META = {
  ...
  aroon: {
    name: 'Пересечение линий Aroon Up/Down',
    desc: [
      'Aroon(25) измеряет, как давно были экстремумы окна. Позиция всегда по линии, что сверху.',
      '<b>Вход:</b>',
    ...
    indicators: [
      { col: 'aroon_up',   label: 'Aroon Up'   },
      { col: 'aroon_down', label: 'Aroon Down' },
      { col: 'atr',        label: 'ATR'        },
    ],
  },
  default_hedge: {
...
const STREAM_STRATEGY_OPTIONS = [
  ...
  ['aroon',                'Пересечение линий Aroon Up/Down'],
        ...
        <td class="st-actions">
          ...
          <span class="st-more-wrap admin-only">
            ...
            <span class="st-menu hidden" data-stream-id="${s.id}">
              <button onclick="openStreamForm('${s.id}'); closeStreamMenus();">
              ...
              <button class="danger" onclick="deleteStream('${s.id}'); closeStreamMenus();">
