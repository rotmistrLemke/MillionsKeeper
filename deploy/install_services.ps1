...
# Учётка, под которой работает сервис. ВАЖНО: должна совпадать с пользователем,
# у которого в %APPDATA%\MetaQuotes настроен профиль MT5. Иначе LocalSystem
# не найдёт сохранённого счёта → бот не сможет залогиниться.
# Формат: ".\Administrator" (локальная учётка) или "DOMAIN\User".
# Оставьте пустым, чтобы оставить дефолт NSSM (LocalSystem).
$ServiceUser     = ".\Administrator"
$ServicePassword = ""              # пустая строка → запросит интерактивно
...
# Учётка сервиса. По умолчанию nssm запускает под LocalSystem, у которой нет
# профиля MT5 в %APPDATA% — бот не сможет залогиниться. Запускаем под тем же
# юзером, что настраивал MT5 через RDP.
if ($ServiceUser) {
    if (-not $ServicePassword) {
        $sec = Read-Host "Пароль для $ServiceUser" -AsSecureString
        $ServicePassword = [System.Net.NetworkCredential]::new("", $sec).Password
    }
    Invoke-Nssm set TradingHouse ObjectName $ServiceUser $ServicePassword
}
