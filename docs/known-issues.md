...
- **Статус:** требует проверки на реальных данных.
- **Следующий шаг:** после генерации `tests/fixtures/xauusd_h1.csv` прогнать golden-слой и посмотреть фактические TP/exit_reason для `fibonacci_retracement` на реальных барах; если на реальных откатах TP всё равно < entry — это баг сайзинга/TP, иначе закрыть находку.
...
- **Статус:** реальное расхождение контракта и реализации.
- **Следующий шаг:** либо реализовать gap (считать дельту в барах между тестами и пропускать слишком близкие), либо убрать `min_gap_bars` из контракта/документации. Перед правкой — добавить поведенческий тест на 3-touch с близкими барами (сейчас стратегия покрыта только smoke-тестом, слайс A).
