...
В панели регистратора (или Cloudflare) создайте `A`-запись:

| Тип | Имя        | Значение         | TTL  |
|-----|------------|------------------|------|
| A   | trade      | `<IP сервера>`   | 300  |

После этого `trade.your-domain.com` (или какой выбрали) будет резолвиться в IP.
Дождитесь распространения DNS — обычно 5–15 минут. Проверка из cmd на сервере:

```
nslookup trade.your-domain.com
```
...
- `YOUR_DOMAIN` → `trade.your-domain.com`
