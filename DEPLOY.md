...
В панели регистратора (или Cloudflare) создайте `A`-запись. Сервис работает
на **апексе** `tradinghouse.space`:

| Тип | Имя        | Значение           | TTL  |
|-----|------------|--------------------|------|
| A   | @          | `<IPv4 сервера>`   | 300  |

> ⚠️ **Удалите лишние `AAAA` (IPv6) записи**, которые регистратор/хостер
> мог завести автоматически на парковку. Let's Encrypt при наличии `AAAA`
> валидирует **по IPv6 в первую очередь** — если он ведёт на чужой хост,
> challenge вернёт 404 и сертификат не выдастся (Caddy отдаст самоподписанный).
> Оставляйте `AAAA` только если у VPS реально есть этот IPv6 и Caddy на нём слушает.
>
> Нужен и `www`? Заведите ещё одну `A`-запись на `www` → тот же IP и допишите
> домен в `Caddyfile` через запятую. Имя в `Caddyfile` должно **точно** совпадать
> с тем, что открываете в браузере.
